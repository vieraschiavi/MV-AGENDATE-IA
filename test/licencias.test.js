...
test('crearPedido valida plan y email antes de cobrar nada', () => {
  const sinPlan = crearPedido({ plan: 'inventado', email: 'a@b.com' });
  assert.equal(sinPlan.ok, false);
  const sinEmail = crearPedido({ plan: 'basico' });
  assert.equal(sinEmail.ok, false);
});

test('crearPedido usa el precio del catálogo, nunca uno inventado', () => {
  const r = crearPedido({ plan: 'basico', email: 'cliente@test.com' });
...
test('el plan SaaS siempre es recurrente aunque no se pida explícitamente', () => {
  const r = crearPedido({ plan: 'saas', email: 'cliente@test.com' });
  assert.equal(r.pedido.recurrente, true);
});

test('confirmarPago emite licencia y token solo después del pago, es idempotente', () => {
  const { pedido } = crearPedido({ plan: 'full', email: 'full@test.com' });
  assert.equal(obtenerPedido(pedido.id).estado, 'pendiente');

  const r1 = confirmarPago(pedido.id);
  ...
  const r2 = confirmarPago(pedido.id);
...
test('confirmarPago sobre un pedido inexistente no inventa nada', () => {
  const r = confirmarPago('ORD-NO-EXISTE');
  assert.equal(r.ok, false);
});

test('validarToken solo reconoce tokens de pedidos realmente pagados', () => {
  const { pedido } = crearPedido({ plan: 'basico', email: 'validar@test.com' });
  assert.equal(validarToken('token-inventado-cualquiera'), null, 'sin pagar, ningún token es válido');

  const { pedido: pagado } = confirmarPago(pedido.id);
  const encontrado = validarToken(pagado.token);
  assert.ok(encontrado, 'el token real de un pedido pagado sí valida');
  assert.equal(encontrado.id, pedido.id);
});

test('buscarPedidoPendientePorEmail solo devuelve pendientes recurrentes de MercadoPago de ese email/plan', () => {
  const email = 'recurrente-' + Date.now() + '@test.com';
  const { pedido: a } = crearPedido({ plan: 'saas', email });
  const { pedido: b } = crearPedido({ plan: 'saas', email });
  // Un pedido no-recurrente del mismo email no debe interferir.
  crearPedido({ plan: 'basico', email });
  // Ni uno recurrente de otro email.
  crearPedido({ plan: 'saas', email: 'otro-' + email });

  const idsValidos = new Set([a.id, b.id]);
  const primero = buscarPedidoPendientePorEmail(email, 'saas');
  ...
  confirmarPago(primero.id);
  const restante = [...idsValidos].find((id) => id !== primero.id);
  const segundo = buscarPedidoPendientePorEmail(email, 'saas');
  ...
  confirmarPago(restante);
  assert.equal(buscarPedidoPendientePorEmail(email, 'saas'), null);
});
