...
const Impuestos = ({ sugerido, moneda }) => {
  ...
  const estimar = async () => {
    ...
    try {
      ...
      setResultado({ ok: false, error: t('Error de red.') });
    }
    setCargando(false);
  };
  ...
  return (
    <div className="card wide" style={{ marginTop: 16 }}>
      <h2 className="mv-h">{t('🧾 Neto estimado según los impuestos de tu país')}</h2>
      <p style={{ fontSize: '.85rem', color: 'var(--muted)', margin: '0 0 10px' }}>
        {t('La IA estima tu carga impositiva (régimen simplificado, aportes) según la ley de tu país configurado y calcula cuánto te queda neto. Orientativo — no reemplaza a tu contador.')}
      </p>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        <input
          type="number" placeholder={`${t('Facturación mensual')}${sugerido ? ` (${t('sugerido:')} ${Math.round(sugerido)})` : ''}`}
        ...
        <button className="btn cel" onClick={estimar} disabled={cargando}>
          {cargando ? t('Estimando…') : t('Estimar impuestos')}
        </button>
      </div>
      ...
      {resultado?.ok && (
        <div style={{ marginTop: 12 }}>
          <div style={{ fontSize: '.9rem' }}>
            <strong>{resultado.pais}</strong> · {t('Régimen sugerido:')} <strong>{resultado.regimen_sugerido}</strong>
            {resultado.fuente === 'ia' ? ` ${t('· calculado con IA')}` : ` ${t('· guía local aproximada')}`}
          </div>
          <div className="mv-tablewrap" style={{ marginTop: 8 }}>
            <table className="mv-table">
              <tbody>
                <tr><td>{t('Facturación bruta')}</td><td style={{ textAlign: 'right' }}>{dinero(resultado.ingresos_brutos)} {moneda}</td></tr>
                {resultado.detalle.map((d, i) => (
                  <tr key={i}><td style={{ color: 'var(--muted)' }}>− {d.concepto}</td><td style={{ textAlign: 'right', color: 'var(--danger)' }}>{dinero(d.monto)}</td></tr>
                ))}
                <tr><td><strong>{t('Neto estimado')}</strong></td><td style={{ textAlign: 'right' }}><strong style={{ color: '#1a7f37' }}>{dinero(resultado.neto_estimado)} {moneda}</strong></td></tr>
              </tbody>
            </table>
          </div>
...
export default function Dashboards() {
  ...
  if (!datos) return <div className="mv-content"><div className="mv-empty">{t('Cargando…')}</div></div>;
  ...
  const Delta = ({ v }) => (v === 0 ? null : <span className={`d ${v > 0 ? 'up' : 'down'}`}>{v > 0 ? '▲ +' : '▼ '}{v} {t('vs mes ant.')}</span>);
  const q = qs();
  ...
  return (
    <>
      <div className="mv-pagehead">
        <div><div className="mv-crumb">{t('Espacio de trabajo')}</div><h1>📊 {t('Dashboards')}</h1></div>
      </div>
      <div className="mv-content">
        <div className="card" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: 16 }}>
          <div><label>{t('Año')}</label><select {...sel('anio')}><option value="">{t('Todos')}</option>{(opciones?.anios || []).map((a) => <option key={a}>{a}</option>)}</select></div>
          <div><label>{t('Mes')}</label><select {...sel('mes')}><option value="">{t('Todos')}</option>{(opciones?.meses || []).map((m) => <option key={m} value={m}>{m}</option>)}</select></div>
          <div><label>{t('Oficio')}</label><select {...sel('oficio')}><option value="">{t('Todos')}</option>{(opciones?.oficios || []).map((o) => <option key={o}>{o}</option>)}</select></div>
          <div><label>{t('Estado')}</label><select {...sel('estado')}><option value="">{t('Todos')}</option>{(opciones?.estados || []).map((e) => <option key={e}>{e}</option>)}</select></div>
          {multi && <div><label>{t('Profesional')}</label><select {...sel('profesionalId')}><option value="">{t('Todos')}</option>{opciones.profesionales.map((p) => <option key={p.id} value={p.id}>{p.nombre}</option>)}</select></div>}
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <a className="btn sec" href={'/api/agenda.csv' + (q ? '?' + q : '')}>⬇️ {t('Agenda CSV')}</a>
            <a className="btn sec" href={'/api/agenda.xls' + (q ? '?' + q : '')}>⬇️ {t('Agenda Excel')}</a>
            <a className="btn sec" href="/api/clientes.xls">⬇️ {t('Clientes Excel')}</a>
          </div>
        </div>

        <div className="kpis">
          {[
            [t('Trabajos totales'), d.total, null],
            [t('Completados'), d.completadas, null],
            [t('Cancelados'), d.canceladas, null],
            [t('Sueldo total (completados)'), dinero(d.sueldo_total || 0), null],
            [t('Ticket promedio'), dinero(d.ticket_promedio || 0), null],
            [t('Trabajos (12m)'), s.total_trabajos, <Delta key="t" v={deltaT} />],
            [t('Facturado (12m)'), dinero(s.total_facturado), <Delta key="f" v={deltaF} />],
          ].map(([l, n, delta]) => (
        ...
        <div className="card wide">
          <h2 className="mv-h">{t('📈 Evolución mensual — trabajos y facturación (12 meses)')}</h2>
          ...
          <div className="leg" style={{ display: 'flex', gap: 14, fontSize: '.76rem', marginTop: 8 }}>
            <span><i style={{ width: 12, height: 12, borderRadius: 3, display: 'inline-block', background: '#1f7ae0', marginRight: 5 }} />{t('Trabajos')}</span>
            <span><i style={{ width: 12, height: 12, borderRadius: 3, display: 'inline-block', background: '#2ea043', marginRight: 5 }} />{t('Facturación (escala propia)')}</span>
          </div>
        </div>
