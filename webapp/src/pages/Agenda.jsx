...
export default function Agenda() {
  ...
  useEffect(() => {
    ...
    fetch('/api/cotizar', {
      ...
      if (vivo) setPreview(r.total != null ? `Presupuesto estimado: ${dinero(r.total)} (${r.duracion_estimada_min} min)` : '');
    });
    return () => { vivo = false; };
  ...
  const guardar = async () => {
    if (!form.clienteNombre || !form.fecha || !form.inicio) return alert('Completá al menos cliente, fecha y hora.');
  ...
  return (
    <>
      <div className="mv-pagehead">
        <div><div className="mv-crumb">Espacio de trabajo</div><h1>📅 Agenda</h1></div>
      ...
      <div className="mv-content">
        <div className="mv-pagehead" style={{ background: 'none', border: 0, padding: '0 0 14px', flexWrap: 'wrap' }}>
          ...
          <button className="btn cel" onClick={() => setModal(true)}>+ Nueva cita</button>
        ...
        {vista === 'tabla' ? (
          <div className="mv-tablewrap">
            {citas.length === 0 ? <div className="mv-empty">Sin citas con ese filtro.</div> : (
              <table className="mv-table">
                <thead>
                  <tr>
                    <th>Cliente</th><th>Trabajo</th><th>Oficio</th>
                    {multi && <th>Profesional</th>}
                    <th>Fecha</th><th>Hora</th><th>Dirección</th><th>Precio</th><th>Estado</th><th></th>
                  </tr>
                </thead>
                <tbody>
                  {citas.map((c) => (
                    <tr key={c.id} className="row" onClick={() => ficha(c.id)}>
                      ...
                      <td><span className={`pill ${c.estado}`}><i />{c.estado}</span></td>
            ...
            {ESTADOS.map((e) => {
              ...
              return (
                <div className="mv-board-col" key={e}>
                  <h3><span className={`pill ${e}`}><i />{e}</span><span className="cnt">{cs.length}</span></h3>
                  {cs.map((c) => (
                  ...
                  {cs.length === 0 && <div className="mv-empty" style={{ padding: 8 }}>Vacío</div>}
                </div>
              );
            })}
          </div>
      ...
      {modal && (
        <div className="modal" style={{ display: 'grid' }}>
          <div className="box">
            <h3>Nueva cita</h3>
            <div className="form2">
              <div><label>Cliente</label><input value={form.clienteNombre} onChange={(e) => of('clienteNombre', e.target.value)} /></div>
              ...
              <div><label>Fecha</label><input type="date" value={form.fecha} onChange={(e) => of('fecha', e.target.value)} /></div>
              <div><label>Hora inicio</label><input type="time" value={form.inicio} onChange={(e) => of('inicio', e.target.value)} /></div>
              <div><label>Hora fin</label><input type="time" value={form.fin} onChange={(e) => of('fin', e.target.value)} /></div>
              <div><label>Distancia estimada (km)</label><input type="number" value={form.distanciaKm} onChange={(e) => of('distanciaKm', e.target.value)} /></div>
            </div>
            <label>Dirección</label><input style={{ width: '100%' }} value={form.direccion} onChange={(e) => of('direccion', e.target.value)} />
            <label>Quién atiende (si no es el titular)</label><input style={{ width: '100%' }} value={form.receptor} onChange={(e) => of('receptor', e.target.value)} />
            {preview && <div style={{ marginTop: 10, fontSize: '.85rem', color: 'var(--muted)' }}>{preview}</div>}
            <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
              <button className="btn cel" onClick={guardar}>Guardar cita</button>
