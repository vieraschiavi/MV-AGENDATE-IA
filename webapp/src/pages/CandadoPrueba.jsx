...
export default function CandadoPrueba() {
  ...
  const activar = async (e) => {
    ...
    if (r.ok && d.ok) { setMsg('✅ ¡Licencia activada! Gracias por tu compra.'); setTimeout(() => window.location.reload(), 1200); }
    else setMsg(d.error || 'No se pudo activar el código. Revisá que sea el que te llegó al comprar.');
  };
  ...
  if (!estado.vencida) {
    return (
      <div style={{
        ...
        <a href="/comprar.html" style={{ color: '#c2410c', fontWeight: 700 }}>Comprá tu licencia →</a>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ background: '#fff', borderRadius: 16, maxWidth: 460, width: '100%', padding: 28, textAlign: 'center' }}>
        ...
        <h2 style={{ color: '#0f2a43', margin: '0 0 8px' }}>Tu prueba gratis terminó</h2>
        <p style={{ color: '#5a6b7c', fontSize: '.92rem', margin: '0 0 18px' }}>
          Gracias por probar MV Agendate IA. Para seguir usándolo, comprá tu licencia
          (pago único) y activala acá con el código que te llega al pagar.
        </p>
        ...
        <form onSubmit={activar} style={{ display: 'grid', gap: 8 }}>
          <input value={codigo} onChange={(e) => setCodigo(e.target.value)} placeholder="Código de licencia (te llegó al comprar)"
