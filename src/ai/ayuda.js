...
export const TEMAS_AYUDA = [
  ...
  },
  {
    ...
    texto: 'Para que el asistente también atienda llamadas necesitás tu PROPIA cuenta de Twilio (es de Twilio, no nuestra — la creás gratis en twilio.com). Pasos: 1) Creá la cuenta y salí del modo prueba agregando un método de pago; con la prueba, Twilio antepone un mensaje grabado a cada llamada, y eso arruina una demo real. 2) En Configuración → Canales, pegá tu Account SID y tu Auth Token de Twilio — quedan en tu programa, nunca se comparten con nosotros. 3) Desde esa misma pantalla, buscá y activá un número: el programa lo compra y lo deja apuntando a tu asistente solo, no hace falta tocar la consola de Twilio para nada de eso. El paso que más traba a la gente no es técnico: en Uruguay y buena parte de LATAM, un número LOCAL con capacidad de voz pide documentación antes de habilitarse (lo pide Twilio, por regulación de cada país) — si se complica, un número de EE.UU./Canadá no pide ese trámite, aunque le queda como número extranjero a quien te llama. Los minutos y el alquiler del número los paga cada profesional en su propia cuenta de Twilio, no es un costo del software. Existe además una versión con voz más natural y en tiempo real, pero esa necesita que el programa esté corriendo en tu computadora o un servidor propio: en la versión online gratuita funciona la versión estándar, que ya suena bien.'
  },
  {
